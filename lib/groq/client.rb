...
class Groq::Client
  ...
  def chat(messages, model_id: nil, raw: false, tools: nil, tool_choice: nil, max_tokens: nil, temperature: nil, json: false, stream: nil, &stream_chunk)
    ...
    if response.body.is_a?(Hash)
      return raw ? response : response.body.dig("choices", 0, "message")
    end
    response.body
  end
